...
asynStatus Kreios::defineSpectrumSFAT()
{
    ...
    std::map<std::string, std::string> data;
    std::stringstream cmd;
    double startEnergy, endEnergy, dwellTime;
    ...
    // SFAT (snapshot): step width and pass energy are computed automatically by
    // Prodigy from the detector calibration, so only Samples is sent (spec 2.4).
...
asynStatus Kreios::checkSpectrumSFAT()
{
    ...
    // SFAT (snapshot): step width and pass energy are computed automatically by
    // Prodigy from the detector calibration, so only Samples is sent (spec 2.9).
...
asynStatus Kreios::setAnalyzerParameterDirectly()
{
    ...
    cmd << " Polarity:\"" << (polarity == 0 ? "negative" : "positive") << "\"";
                                            ...
                                            std::string &value)
{
    ...
    asynStatus status = commandResponse(cmd.str(), response, data);
    if (status == asynSuccess && data.count("Value") > 0) {
        value = data["Value"];
    }
    return status;
}
